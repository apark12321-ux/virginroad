    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
      ...
      <div className="grid lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="space-y-5">
            ...
            <label className="flex items-start gap-3 cursor-pointer py-1">
              ...
              <span className="text-[13.5px] text-[#2C2419] leading-[1.5]">
                ...
                <span className="block text-[12px] text-[#968670] mt-0.5">자녀 우대와 비교해 더 큰 쪽만 적용됩니다</span>
              </span>
            </label>
            ...
            <div>
              ...
              <div className="mt-2">
                <label className="block text-[12px] text-[#6B5D4A] mb-1">
                  납입 회차: <span className="font-medium text-[#4A3F30]">{input.jeongyakMonths}회</span>
                </label>
                <input
                  type="range"
                  min={0}
            ...
            <label className="flex items-start gap-3 cursor-pointer py-1">
              ...
              <span className="text-[13.5px] text-[#2C2419] leading-[1.5]">
                ...
                <span className="block text-[12px] text-[#968670] mt-0.5">국토부 부동산거래 전자계약시스템(irds.kr) 이용 시</span>
              </span>
            </label>
            ...
            <label className="flex items-start gap-3 cursor-pointer py-1">
              ...
              <span className="text-[13.5px] text-[#2C2419] leading-[1.5]">
                ...
                <span className="block text-[12px] text-[#968670] mt-0.5">자기자금 충분히 확보해 소액 대출하는 경우</span>
              </span>
            </label>
          </div>
        </section>
        ...
        <section className="space-y-4">
          <div className="bg-gradient-to-br from-[#FFE9D9] to-[#FFF6EE] border border-[#FFD2BD] rounded-[14px] p-6 sm:p-7">
            <div className="text-[12px] font-bold text-[#B0432F] uppercase tracking-[0.1em] mb-2">
              최종 적용 예상 금리
            </div>
            <div className="flex items-baseline gap-3">
              <span className="text-[44px] sm:text-[52px] font-bold text-[#1F1A12] tracking-[-0.03em] leading-none">
                {result.finalRate.toFixed(2)}
              </span>
              <span className="text-[20px] font-bold text-[#1F1A12]">%</span>
              {result.hitFloor && (
            ...
            <div className="text-[13px] text-[#6B5D4A] mt-2">
          ...
          <div className="bg-white border border-[#E8DDCB] rounded-[14px] p-5 sm:p-6">
            <h3 className="text-[15px] font-bold text-[#1F1A12] mb-4">단계별 인하 시뮬레이션</h3>
            <div className="space-y-2.5 text-[13.5px]">
              <div className="flex justify-between items-center pb-2.5 border-b border-[#F4EDE3]">
                <span className="text-[#4A3F30]">기본금리 ({input.income === "low" ? "저소득" : input.income === "mid" ? "중간소득" : "고소득"} · {input.loanTerm}년)</span>
                <span className="font-bold text-[#1F1A12]">{result.baseRate.toFixed(2)}%</span>
              </div>
              <div className="flex justify-between items-center text-[13px]">
                <span className="text-[#6B5D4A]">▼ {result.childPart.reason}</span>
                <span className={result.childPart.rate > 0 ? "font-medium text-[#B0432F]" : "text-[#968670]"}>
                  {result.childPart.rate > 0 ? `-${result.childPart.rate.toFixed(2)}%p` : "—"}
                </span>
              </div>
              <div className="flex justify-between items-center text-[13px]">
                <span className="text-[#6B5D4A]">▼ {result.jyPart.reason}</span>
                <span className={result.jyPart.rate > 0 ? "font-medium text-[#B0432F]" : "text-[#968670]"}>
                  {result.jyPart.rate > 0 ? `-${result.jyPart.rate.toFixed(2)}%p` : "—"}
                </span>
              </div>
              <div className="flex justify-between items-center text-[13px]">
                <span className="text-[#6B5D4A]">▼ 부동산 전자계약</span>
                <span className={result.ePart > 0 ? "font-medium text-[#B0432F]" : "text-[#968670]"}>
                  {result.ePart > 0 ? `-${result.ePart.toFixed(2)}%p` : "—"}
                </span>
              </div>
              <div className="flex justify-between items-center text-[13px]">
                <span className="text-[#6B5D4A]">▼ 30% 이하 신청</span>
                <span className={result.u30Part > 0 ? "font-medium text-[#B0432F]" : "text-[#968670]"}>
                  {result.u30Part > 0 ? `-${result.u30Part.toFixed(2)}%p` : "—"}
                </span>
              </div>
              <div className="flex justify-between items-center pt-2.5 border-t border-[#FFE9D9]">
                <span className="font-bold text-[#1F1A12]">최종 금리</span>
          ...
          <div className="bg-white border border-[#E8DDCB] rounded-[14px] p-5 sm:p-6">
            <h3 className="text-[15px] font-bold text-[#1F1A12] mb-4">월 상환액 (원리금균등)</h3>
            <div className="space-y-3">
              <div className="flex justify-between items-baseline">
                <span className="text-[13px] text-[#6B5D4A]">우대 적용 후 월 상환액</span>
                <span className="font-bold text-[20px] text-[#1F1A12] tracking-[-0.02em]">
                  {Math.round(result.monthly).toLocaleString()}원
                </span>
              </div>
              <div className="flex justify-between items-baseline text-[12.5px] text-[#968670]">
                <span>기본금리 적용 시</span>
                <span>{Math.round(result.baseMonthly).toLocaleString()}원</span>
              </div>
              <div className="pt-3 mt-3 border-t border-[#F4EDE3] bg-[#FFF6EE] -mx-5 sm:-mx-6 px-5 sm:px-6 pb-3 rounded-b-[14px]">
                <div className="flex justify-between items-baseline">
      ...
      <section className="mt-10 bg-[#FAF7F2] border border-[#E8DDCB] rounded-[12px] p-5 sm:p-6">
        ...
        <div className="space-y-2.5 text-[13px] text-[#6B5D4A] leading-[1.7]">
          <p>
            본 계산기는 <strong className="text-[#4A3F30]">한국주택금융공사 2026년 5월 1일 공시 기준</strong>과 주택도시기금 디딤돌대출 안내 자료를 토대로 작성됐습니다.
            기본금리·우대금리 적용 폭은 정책에 따라 수시로 변경되며, 본인의 신용평점·담보 주택 평가·소득 산정 결과에 따라
            실제 적용 금리는 본 계산 결과와 다를 수 있습니다.
          </p>
          <p>
            <strong className="text-[#4A3F30]">계산기 한계:</strong> 자녀 우대금리는 2025년 3월 24일 신규 접수분부터
